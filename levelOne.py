...
from battleSystemSkeleton import *  
...
pygame.display.set_caption("The Battle Of The Marauders")
WIDTH, HEIGHT = 800, 600

# set frames per second and player velocity
FPS = 60
PLAYER_VEL = 5
...
def load_sprite_sheets(dir1, dir2, width, height, direction=False):
    ...
    # create a dictionary to hold all the sprites
    all_sprites = {}
    # loop through the images and load them into surfaces and add them to the dictionary
    for image in images:
        ...
        # if direction is true, add sprites to the dictionary for both left and right directions
        if direction:
            all_sprites[image.replace(".png", "") + "_right"] = sprites
            all_sprites[image.replace(".png", "") + "_left"] = flip(sprites)
        else:
            all_sprites[image.replace(".png", "")] = sprites
    # return the dictionary of all the loaded sprites
    return all_sprites

# function to get terrain blocks
def get_block(size):
    ...
    # blit the terrain image onto the surface and return it
    surface.blit(image, (0, 0), rect)
    return pygame.transform.scale2x(surface)

actor = "Archer"
def setLvl1Character(choice):
    global actor
    if choice == "Wizard":
        actor = choice
    elif choice == "Archer":
...
class Player(pygame.sprite.Sprite):
    ...
    # define player's color, gravity, sprites, and animation delay
    COLOR = (255, 0, 0)
    GRAVITY = 1
    ...
    def __init__(self, x, y, width, height):
        ...
        if actor == "Wizard":
            self.SPRITES = load_sprite_sheets("MainCharacters", "Wizard", 32, 32, True)
        elif actor == "Archer":
            self.SPRITES = load_sprite_sheets("MainCharacters", "Archer", 32, 32, True)
        else:
            self.SPRITES = load_sprite_sheets("MainCharacters", "Knight", 32, 32, True)
...
class Sword(Object):
    ...
    def __init__(self, x, y, width, height):
        super().__init__(x, y, width, height, "sword")
        self.sword = load_sprite_sheets("Traps", "Sword", width, height)
        self.image = self.sword["swords"][0]
        self.mask = pygame.mask.from_surface(self.image)
        self.animation_count = 0
        self.animation_name = "swords"
    ...
    def loop(self):
        sprites = self.sword[self.animation_name]
...
def handle_move(player, objects):
    ...
    for obj in to_check:
        if obj and obj.name == "sword":
            player.make_hit()
            battleSystemSkeletonPage(actor,20)
                


def main(window):
    clock = pygame.time.Clock()
    background, bg_image = get_background("level_1.png")
    ...
    objects = [
        #x,y
        #Block(block_size * 0, HEIGHT - block_size * 1, block_size), #Bottom left of the screen
        #Block(block_size * 0, HEIGHT - block_size * 6, block_size), #Top left of the screen

        Block(block_size * 1, HEIGHT - block_size * 2, block_size), #Player Lands
